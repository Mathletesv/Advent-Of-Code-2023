...
int config(vector<string> grid, int x, int y, int dir) {
  queue<state> bfs;
...
int main() {
  ...
  for (int i = 0; i < grid.size(); i++) {
    best = max(config(grid, 0, i, 1), best);
    best = max(config(grid, grid[0].size() - 1, i, 3), best);
  }
  for (int i = 0; i < grid[0].size(); i++) {
    best = max(config(grid, i, 0, 2), best);
    best = max(config(grid, i, grid.size() - 1, 0), best);
  }
